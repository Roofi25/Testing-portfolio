...
namespace System_wspomagajacy_organizacje_wydarzen_sportowych.Pages.AddSportFacility
{
    public class IndexModel : PageModel
    {
        public WydarzenieSportowe WydarzenieSportowe { get; set; } = null!;
		public IList<string> NazwyObiektowSportowych { get; set; }
		public IList<string> NumeryBudynkowObiektowSportowych { get; set; }
		[BindProperty]
		...
		public IActionResult OnPost()
		{
			...
			try
			{
				using (SqlConnection connection = new SqlConnection(connectionString))
				{
					...
					string getSportFacilitiesNames = "SELECT Nazwa FROM ObiektSportowy";

					using (SqlCommand command = new SqlCommand(getSportFacilitiesNames, connection))
					{
						using (SqlDataReader reader = command.ExecuteReader())
						{
							NazwyObiektowSportowych = new List<string>();
							string nazwa;
							while (reader.Read())
							{
								nazwa = reader.GetString(0);
								NazwyObiektowSportowych.Add(nazwa);
							}
						}
					}

					string getSportFacilitiesBuildingNumbers = "SELECT NumerBudynku FROM ObiektSportowy";

					using (SqlCommand command = new SqlCommand(getSportFacilitiesBuildingNumbers, connection))
					{
						using (SqlDataReader reader = command.ExecuteReader())
						{
							NumeryBudynkowObiektowSportowych = new List<string>();
							string numerBudynku;
							while (reader.Read())
							{
								numerBudynku = reader.GetString(0);
								NumeryBudynkowObiektowSportowych.Add(numerBudynku);
							}
						}
					}

					if (NazwyObiektowSportowych.Contains(Nazwa))
					{
						TempData["ErrorMessage"] = "Obiekt sportowy o takiej nazwie ju¿ istnieje w bazie danych!";
						return RedirectToPage();
					}

					if (NumeryBudynkowObiektowSportowych.Contains(NumerBudynku))
					{
						TempData["ErrorMessage"] = "Obiekt sportowy o takim numerze budynku ju¿ istnieje w bazie danych!";
						return RedirectToPage();
					}

					string addSportFacility = "INSERT INTO ObiektSportowy(Nazwa, Miejscowosc, Ulica, NumerBudynku, KodPocztowy) VALUES(@nazwa, @miejscowosc, @ulica, @numerBudynku, @kodPocztowy)";

					using(SqlCommand command = new SqlCommand(addSportFacility, connection))
					{
					...
					TempData["SuccessMessage"] = "Uda³o Ci siê dodaæ obiekt sportowy!";
