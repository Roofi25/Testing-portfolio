...
namespace System_wspomagajacy_organizacje_wydarzen_sportowych.Pages.AddTreatFacility
{
    public class IndexModel : PageModel
    {
		public WydarzenieSportowe WydarzenieSportowe { get; set; } = null!;
		public IList<string> NazwyLokaliGastronomicznych { get; set; }
		public IList<string> NumeryBudynkowLokaliGastronomicznych { get; set; }
		[BindProperty]
		...
		public IActionResult OnPost()
		{
			...
			try
			{
				using (SqlConnection connection = new SqlConnection(connectionString))
				{
					...
					string getTreatFacilitiesNames = "SELECT Nazwa FROM LokalGastronomiczny";

					using (SqlCommand command = new SqlCommand(getTreatFacilitiesNames, connection))
					{
						using (SqlDataReader reader = command.ExecuteReader())
						{
							NazwyLokaliGastronomicznych = new List<string>();
							string nazwa;
							while (reader.Read())
							{
								nazwa = reader.GetString(0);
								NazwyLokaliGastronomicznych.Add(nazwa);
							}
						}
					}

					string getTreatFacilitiesBuildingNumbers = "SELECT NumerBudynku FROM LokalGastronomiczny";

					using (SqlCommand command = new SqlCommand(getTreatFacilitiesBuildingNumbers, connection))
					{
						using (SqlDataReader reader = command.ExecuteReader())
						{
							NumeryBudynkowLokaliGastronomicznych = new List<string>();
							string numerBudynku;
							while (reader.Read())
							{
								numerBudynku = reader.GetString(0);
								NumeryBudynkowLokaliGastronomicznych.Add(numerBudynku);
							}
						}
					}

					if (NazwyLokaliGastronomicznych.Contains(Nazwa))
					{
						TempData["ErrorMessage"] = "Lokal gastronomiczny o takiej nazwie ju¿ istnieje w bazie danych!";
						return RedirectToPage();
					}

					if (NumeryBudynkowLokaliGastronomicznych.Contains(NumerBudynku))
					{
						TempData["ErrorMessage"] = "Lokal gastronomiczny o takim numerze budynku ju¿ istnieje w bazie danych!";
						return RedirectToPage();
					}

					string addTreatFacility = "INSERT INTO LokalGastronomiczny(Nazwa, Miejscowosc, Ulica, NumerBudynku, KodPocztowy) VALUES(@nazwa, @miejscowosc, @ulica, @numerBudynku, @kodPocztowy)";

					using (SqlCommand command = new SqlCommand(addTreatFacility, connection))
					{
					...
					TempData["SuccessMessage"] = "Uda³o Ci siê dodaæ lokal gastronomiczny!";
